...
fig1, axes1 = plt.subplots(1, LenListParam, figsize=(30, 10))
fig2, axes2 = plt.subplots(1, LenListParam, figsize=(30, 10))
step_plot = n // 5
plot_name = '3B.png'
...
for ia, Da in enumerate(ListParam): # 0.007, 0.008
    ...
    for i in tqdm(range(n), total=n):
        ...
        if i % step_gif == 0:
            ...
            AB1 -= digit_borders(digit)[:,:,np.newaxis]
            AB2 += (1-digit[:,:,np.newaxis])
            AB2 -= digit_borders(digit)[:,:,np.newaxis]
...
# save the plot fig
plt.tight_layout()
fig1.savefig(plot_name + '1.png')
fig2.savefig(plot_name + '2.png')
plt.close()